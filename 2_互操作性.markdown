...
*互操作性*能让Swift与Objective-C双向互为接口，使你能够在一种语言中访问和使用另一种语言编写的代码。在开始把Swift整合进开发工作流之前，先理解如何利用互操作性来改善、改进和增强Cocoa应用的开发方式是个好主意。
...
要编写设计优秀，适应性好的应用的一条准则是使用成熟的Cocoa设计模式。许多设计模式都依赖在Objective-C中定义的类。得益于Swift与Objective-C的互操作性，你可以在你的Swift代码中使用这些设计模式。而且在很多情况下，你可以使用Swift的语言特性来增强或简化已有的Cocoa设计模式，使它们更加强大且更加易用。

### 委托

（译者注：这里把delegation翻译成委托，如果你学过.NET，请不要与.NET的委托混淆，两者是完全不同的概念。）

无论是在Swift还是Objective-C中，委托（delegation）通常是通过定义了交互方式的协议来表示的。类通过遵守协议来实现委托。在Objective-C中，在你向委托对象发送一个它可能无法响应的消息之前，你需要先询问委托对象是否可以响应这个方法选择器（selector）。在Swift中，你可以使用可选调用链来对可能为nil的对象，触发一个可选的协议方法，然后通过使用`if-let`语法来的到可能的返回结果。后面列出的代码展示了如下几个过程：

1. 检查`myDelegate`是否不为`nil`。
...
一个*懒惰属性*（lazy property）是指其底层的值在首次访问的时候才倍初始化的属性。懒惰属性在属性的初始值需要复杂运算，或者需要进行大计算量地设置，或者是在实例初始化完成前无法确知的情况下非常有用。

在Objective-C中，属性可以覆盖取值方法的默认实现，让属性对应的实例变量的值为`nil`的时候，才有条件地进行初始化：
...
在Swift中，带有初始值的存储属性（stored property）可以使用`lazy`修饰符声明，以让计算初始值的表达式在属性首次访问的时候才进行计算：
...
懒惰属性在初次访问时才进行计算，而此时实例已经初始化完成，因此默认的初始化表达式可以访问实例的常量或变量属性：
...
要了解更多关于“懒惰存储属性”的信息，请查看Swift编程语言（Swift 2.1版）。
...
> **提示**
>
> *处理*（consume）错误的方法，比如委托方法或接受一个包含`NSError`参数的完成代码快（completion handler）作为参数的方法，在导入Swift的时候不会被转译成可以抛出错误的方法。
...
如果Objective-C方法的最后一个非代码块参数的类型为`NSError **`，Swift就会把它替换为`throws`关键词，以指示这个方法能抛出一个错误。如果Objective-C方法的错误参数是第一个参数，Swift会试图通过删除方法选择器第一部分的`WithError`或`AndReturnError`词尾（如果存在的话）来进一步简化方法名。如果得到的方法名，已经被另一个方法所使用，那么这个方法名将不会改变。
...
例如，`NSFileManager`的实例方法：`URLForDirectory(_:inDomain:appropriateForURL:create:)`，返回一个位于特定搜索路径和文件域（domain）下的URL，或者如果合适的URL不存在且无法创建，则产生一个错误。在Objective-C中，方法调用的成败可以通过检查返回值是否为一个`NSURL`对象来确定。
...
如果Objective-C方法中产生了错误，改方法的错误指针就回指向这个错误：
...
如果在Swift方法中发生了错误，这个错误就回被抛出，并自动传递给调用者：
...
如果方法无法使用文档的文件内容创建一个对象，它会抛出一个`NSError`对象。如果方法是在Swift代码中调用的，错误会被传递给它的调用作用于。如果方法是在Objective-C代码中调用的，错误就回传递给错误指针参数。

（译者按：原文中这段文字于本节开头第一段重复，这里不再翻译）

> **提示**
>
> 尽管Swift的错误处理于Objective-C的异常处理类似，但是这两个是完全不同的功能。如果Objective-C方法在运行时抛出异常Swift会触发一个运行时错误。在Swift中是无法直接从Objective-C的异常中恢复的。所有的异常都必须在Swift调用的Objective-C代码实现中处理。

（译者按：Swift和Objective-C的错误（error）类似于其他语言（例如Java，C#，Ruby，Python等）的异常，而Objective-C的异常（exception）则类似于其他语言的运行时错误。）

### 键值观察

键值观察（key-value observing）是一种能在指定的属性发生变化的时候通知其他对象的机制。只要Swift类是继承自`NSObject`的，它就可以使用键值观察。你可以通过如下三个步骤在Swift中实现键值观察。

1. 为你要观察的属性加上`dynamic`修饰符。要了解更多关于`dynamic`的信息，请查看[使用动态分配](#使用动态分配)部分。
...
`NSUndoManager`支持两种注册撤销操作的方式：一种是“简单撤销”，这种方式会执行一个带有一个对象参数方法。另一种是“基于动作对象的撤销”，这种方式能使用可接受任意数量、任意类型参数的`NSInvocation`对象，执行撤销操作。
...
对于接受多个参数的方法，你可以使用`NSInvocation`创建一个撤销操作，用于触发一个接受多个参数的方法，把应用恢复操作前的状态：
...
`prepareWithInvocationTarget(_:)方法返回指定`target`的一个代理对象。通过把它转成`ToDoListViewController`，使它能直接响应`markTask(_:asCompleted)`调用。

要了解更多信息，请参阅“The Undo Architecture Programming Guide”文档。

### 目标-动作

目标-动作模式是Cocoa中一个常用的设计模式，这种模式下，在事件发生时，一个对象会发送消息给另一个对象。目标-动作模式在Swift和Objective-C中极其相似。在Swift中，你使用`Selector`类型来表示Objective-C的方法选择器。要查看关于目标-动作的Swift代码实例，请查看[Objective-C的方法选择器](#Objective-C的方法选择器)部分。
...
在Swift中，你只需使用一个静态属性就能确保这个属性只被懒惰初始化一次，即便是在多个线程中被同时访问：
...
如果你需要在初始化时，做一些额外的设置，你可以为这个全局常量赋值闭包的执行结果：
...
检查或转换协议，与检查和转换类使用完全相同的语法。下面就是一个使用`as?`操作符检查对象是否遵守协议的一个例子：
...
这个能处理`Venue`实例的应用，在和网络服务器通讯的时候，获得了JSON格式的地点数据，如下：
...
包含多个可选类型值绑定表达式的`guard`语句保证了`attributes`参数从预期的文件格式中获得了所有必要的信息。如果任意一个可选类型值绑定表达式在为常量赋值时失败了，`guard`语句会立刻停止执行其他语句，并转跳到`else`分支，返回`nil`。
...
在前面的例子中，`Venue`的构造函数在所有的必须的信息都提供的情况下，可选地返回一个实例。否则，构造器会简单的返回`nil`。

如果可以知道一组给定的数据为何不能创建有效实例的原因，通常会很有用。要实现这一点，你需要把这个可失败的构造方法重构为一个可抛出错误的构造方法：
...
这个构造方法没有用一条`guard`语句处理所有`attributes`的值，而是分别检查了每个值，并在值不存在或不合法的时候抛出一个错误。

例如，如果JSON没有包含键为`name`的值，那么这个构造器会抛出`ValicationError.Missing`枚举值，并把对应的`"name"`字段作为枚举的关联值：
...
或者，如果JSON包含了所有必需的字段，但是`"category"`键的值不匹配`Category`枚举中定义的值的`rawValue`，那么这个构造方法会抛出`ValicationError.Invalid`枚举值，并把对应的`"category"`字段作为枚举的关联值：
...
某些类和方法并非在你的应用支持的所有版本和所有平台上都可用。要保证你的应用能够你的应用能够使用任何功能的缺失，你需要检查API的可用性。
...
在Swift中，调用一个不是在所有目标平台版本上可用的方法会引发一个编译时错误。
...
如果应用的目标iOS版本早于8.0，OS X的目标版本早于10.0，`requestWhenInUseAuthorization()`方法是不可用的，因此编译器会汇报一个错误。
...
或者，你也可以使用`guard`语句检查可用性，除非当前的运行平台满足要求，否则跳出代码块。这种方法简化了判断不同平台能力的处理逻辑。
...
在OS X上，你通常是通过单击击Dock或LaunchPad上的应用图标、或双击Finder中的应用图标来打开一个应用的。不过，你也可以从终端用程序打开一个应用，并给它传递命令行参数。
...
Swift提供了与C语言基本整数类型--比如，`char`，`int`，`float`，和`double`的等价类型。不过这些类型不会通过隐式转换转成Swift的整形，如`Int`。因此，只有在你的代码对这些类型有特殊需求时才使用它们，否则，请总是使用`Int`。

<table>
	<tr><td>**C类型**</td><td>**Swift类型**</td></tr>
	<tr><td>`bool`</td><td>`CBool`</td></tr>
	<tr><td>`char`, `signed char`</td><td>`CChar`</td></tr>
	<tr><td>`unsigned char`</td><td>`CUnsignedChar`</td></tr>
	<tr><td>`short`</td><td>`CShort`</td></tr>
	<tr><td>`unsigned short`</td><td>`CUnsignedShort`</td></tr>
	<tr><td>`int`</td><td>`CInt`</td></tr>
	<tr><td>`unsigned int`</td><td>`CUnsignedInt`</td></tr>
	<tr><td>`long`</td><td>`CLong`</td></tr>
	<tr><td>`unsigned long`</td><td>`CUnsignedLong`</td></tr>
	<tr><td>`long long`</td><td>`CLongLong`</td></tr>
	<tr><td>`unsigned long long`</td><td>`CUnsignedLongLong`</td></tr>
	<tr><td>`wchar_t`</td><td>`CWideChar`</td></tr>
	<tr><td>`char16_t`</td><td>`CChar16`</td></tr>
	<tr><td>`char32_t`</td><td>`CChar32`</td></tr>
	<tr><td>`float`</td><td>`CFloat`</td></tr>
	<tr><td>`double`</td><td>`CDouble`</td></tr>
</table>
...
不是用NS_ENUM或NS_OPTIONS宏标记的C的枚举类型会被Swift导入为结构体。C枚举的每个成员都被导入为一个类型为结构体的全局只读计算属性--不是成为Swift结构体本身的成员。
...
Swift会为导入的C结构体类型自动生成遵守`Equatable`协议的实现代码。

### 选项集

Swift也会把用`NS_OPTIONS`宏标记的C语言枚举类型，导入为Swift的选项集（option set）。选项集会与枚举类型一样，导入后会移除选项值名字中的前缀。
...
Swift仅部分支持C语言的`union`类型。当导入包含联合体的C的聚合数据类型（C aggregates）是，Swift是无法访问不受支持的字段的。不过如果C或Objective-C API的参数或返回值包含联合体类型，那么它们是可以在Swift中使用的。

（译者按：C aggregates翻译为聚合数据类型是参考了[维基百科](https://zh.wikipedia.org/wiki/%E7%BB%93%E6%9E%84%E4%BD%93_%28C%E8%AF%AD%E8%A8%80%29)）

### 位域

Swift把结构体的位域，比如Foundation框架的`NSDecimal`类型中能找到的那些，导入为计算属性。当访问对应某个位域的计算属性时，Swift会自动把该值与兼容的Swift类型互相转换。
